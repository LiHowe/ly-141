...
public static class UIThreadHelper
{
    ...
    /// <param name="priority">调度优先级</param>
    /// <returns>异步任务</returns>
    public static async Task SafeInvokeAsync(Action action, Action<Exception>? onError = null,
        DispatcherPriority priority = DispatcherPriority.Normal)
    {
        try
        {
        ...
        catch (Exception ex)
        {
            onError?.Invoke(ex);
            // 可以在这里添加日志记录
            Debug.WriteLine($"UI线程操作异常: {ex.Message}");
        }
    }
    ...
    public static bool TryInvokeSync(Action action, TimeSpan? timeout = null)
    {
        ...
        {
            Debug.WriteLine($"同步UI调用失败: {ex.Message}");
            return false;
        }
    }
    ...
    [Obsolete("尽量避免使用同步版本，推荐使用InvokeAsync")]
    public static T TryInvokeSync<T>(Func<T> func, TimeSpan? timeout = null, T defaultValue = default)
    {
        if (Application.Current?.Dispatcher == null)
            try
            {
        ...
        try
        {
            ...
            if (status == DispatcherOperationStatus.Completed) return operation.Result;
    ...
    /// <param name="priority">调度优先级</param>
    /// <returns>异步任务</returns>
    public static async Task DelayedInvokeAsync(Action action, TimeSpan delay,
        DispatcherPriority priority = DispatcherPriority.Normal)
    {
    ...
    /// <param name="priority">调度优先级</param>
    /// <returns>异步任务</returns>
    public static Task BatchInvokeAsync(IEnumerable<Action> actions,
        DispatcherPriority priority = DispatcherPriority.Normal)
    {
        return InvokeAsync(() =>
        {
